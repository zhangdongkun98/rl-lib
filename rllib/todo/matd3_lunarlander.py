...
from method_matd3.matd3 import MATD3 as Method
...
path_pack, logger = cu.basic.create_dir(config, 'matd3_lunar')
...
def main():
    ...
    for i_episode in range(1, max_episodes + 1):
        ...
        for _ in count():
            ...
            action, action_prob = method.select_actions(States(torch.tensor(state, dtype=torch.float32).unsqueeze(0), indices.clone()), masks)
            next_state, reward, done, _ = env.step(action.item())

            # action_prob = torch.tensor(cu.basic.int2onehot(action, config.dim_action), dtype=torch.float32)
            experience = Experience(States(torch.tensor(state, dtype=torch.float32).unsqueeze(0), indices.clone()),
                    action_prob, States(torch.tensor(next_state, dtype=torch.float32).unsqueeze(0), indices.clone()),
        ...
        for _ in range(avg_length):
            ...
            logger.add_scalar('loss/c_loss', c_loss[0], method.train_step)
            if a_loss[0] != None: logger.add_scalar('loss/a_loss', a_loss[0], method.train_step)
