...
print(script_name, os.path.splitext(script_name), script_name == __file__)
...
directory = './ddpg/exp' + str(time.time()) + script_name + args.env_name +'./'
...
class DDPG(object):
    ...
    def update(self):
        ...
        for it in range(args.update_iteration):
            ...
            self.writer.add_scalar('Loss/critic_loss', self.critic_loss, global_step=self.num_critic_update_iteration)
            # Optimize the critic
            ...
            self.writer.add_scalar('Loss/actor_loss', self.actor_loss, global_step=self.num_actor_update_iteration)
        ...
        for i in range(args.max_episode):
            ...
            for t in count():
                ...
                if random.random() < epsilon:
                    real_action = random.randint(0, 3)
                    action = torch.tensor([0.0, 0.0, 0.0, 0.0])
                    action[real_action] = 1.0
                else:
            ...
            agent.writer.add_scalar('epsilon', epsilon, global_step=i)
            agent.writer.add_scalar('reward', total_reward, global_step=i)
