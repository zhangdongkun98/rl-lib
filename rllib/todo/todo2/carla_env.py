...
class Controller(object):
    ...
    def control(self, action, vehicle, route):
        player = vehicle.player
        ...
        return control_out, route_point


class Vehicle(object):
    def __init__(self, player, start, end, route, collision_sensor):
        self.player = player
        self.start = start
        self.end = end
        self.route = route
...
class CarlaEnv(object):
    ...
    def reset(self):
        ...
        for spawn_point in spawn_points:
            ...
            if player != None:
                ...
                vehicle = Vehicle(player, start, end, waypoints, colsensor)

                self.player_list.append(vehicle)

        next_state_list = []
        for index, vehicle in enumerate(self.player_list):
            next_state_list.append(np.array(self.perception(vehicle)))
    ...
    def step(self, actions):
        ...
        for i in range(skip_num):
            for index, vehicle in enumerate(self.player_list):
                control_command, route_point = self.controller.control(actions[index], vehicle, vehicle.route)
                vehicle.player.apply_control(control_command)
                vehicle.clock += 1
                self.draw_waypoint(route_point)
        ...
        for index, vehicle in enumerate(self.player_list):
            ...
            next_reward_list.append(self.cal_reward(collision_done, reach_done, overtime_done, vehicle))
        destroy_list = []
        ...
        return next_state_list, next_reward_list, next_done_list
    ...
    def perception(self, vehicle):
        ...
        for other_vehicle in self.player_list:
            if other_vehicle != vehicle:
                ...
                relX = int(-rol * math.sin(reltheta) + np.sign(-rol * math.sin(reltheta)) * 0.5)/5
                relY = int(-rol * math.cos(reltheta) + np.sign(-rol * math.cos(reltheta)) * 0.5)/5
        ...
        #return np.concatenate((np.concatenate((np.reshape(occ_map, -1), np.reshape(vel_map, -1)), -1), dist), -1)
        return np.concatenate((np.reshape(occ_map, -1), np.reshape(vel_map, -1)), -1)

    def cal_reward(self, collision_done, reach_done, overtime_done, vehicle):
        if collision_done:
            return -5.0
        elif reach_done:
            return 5.0
    ...
    def get_random_spawn_points(self, num):
        ...
        for i in range(num):
            rand_shift = random.random() * 80.0 - 40.0
            spawn_point = carla.Transform()
            spawn_point.location.x = spawn_x[i]
            spawn_point.location.y = spawn_y[i]
            spawn_point.location.z = 0.8
