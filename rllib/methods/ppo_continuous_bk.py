...
class PPO(cu.rl_template.MethodSingleAgent):
    ...
    def update_policy(self):
        ...
        old_states = torch.squeeze(torch.stack(self._replay_buffer.states).to(self.device), 1).detach()
        old_actions = torch.squeeze(torch.stack(self._replay_buffer.actions).to(self.device), 1).detach()
        old_logprobs = torch.squeeze(torch.stack(self._replay_buffer.logprobs), 1).to(self.device).detach()
...
class ActorCritic(cu.rl_template.Model):
    def __init__(self, config, std_action):
        super(ActorCritic, self).__init__(config, model_id=0)

        self.actor = config.net_actor(config)
        self.critic = config.net_critic(config)
